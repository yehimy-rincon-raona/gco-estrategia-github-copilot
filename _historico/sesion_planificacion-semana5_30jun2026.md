...
- **Pub 8** — `Produccion/semana-5_29jun-03jul/pub-08_mar-30jun_(repub-09_mar-11-ago)/canal-anuncios-tips_pub8.md`
  "La fórmula del prompt que funciona (parte 1)": los 5 ingredientes **Rol + Contexto + Tarea + Restricciones + Formato**, anclados en un caso real de GCO — el control de errores de **SQLCODE en COBOL** (Sesión 2 · G4M/G5M). Incluye post, antes/después del prompt, plantilla copiable, 2 respuestas y brief de la tarjeta `S5-Card_pub8-formula`.
- **Pub 9** — `Produccion/semana-5_29jun-03jul/pub-09_jue-02jul_(repub-10_jue-13-ago)/canal-anuncios-tips_pub9.md`
  "La fórmula del prompt (parte 2): Contexto y Restricciones": el orden de referencias (`#selection` → método → `#file` → `@workspace`) y las reglas de código reales de GCO (COBOL col. 7–72, SQL sin LIKE/DELETE/TRUNCATE, .NET C# 12). Puentea hacia `copilot-instructions.md`. Post, 2 respuestas y brief de la tarjeta `S5-Card_pub9-contexto-restricciones`.
...
- Pub 8 y 9 movidas de S4 → **S5** con su nueva ubicación (`semana-5_29jun-03jul/`), contenido y estado (brief listo · tarjeta pendiente en Figma · fecha a confirmar).
...
- **Pub 7** = Caso de uso 2 (bug que no estaba donde parecía / bug fixing) — el "segundo caso". Vive en `Produccion/semana-4_22-26jun/pub-07_jue-25jun_(repub-08_jue-06-ago)/` con brief + 3 tarjetas del tríptico producidas.
...
- Estado tras la respuesta de Nibaldo del 30 jun:
  ...
  - **Video MCP** (pedido por Jonatan): Nibaldo lo acaba de entregar → en producción esta semana.
...
### 6. Reunión semanal con Jonatan (30 jun) — acuerdos
...
- **VPN:** Jonatan habló con Nibaldo el 30 jun; Nibaldo entendió el requerimiento y lo gestiona con el equipo de sistemas. Acceso esperado esta semana o la próxima. **Contingencia:** si tarda, Yehimy pasa el copy a Nibaldo o Víctor para que publiquen en el Teams de GCO.
...
- **Flujo de aprobaciones:** Jonatan no recibe notificación al ser etiquetado en el Teams de Raona. Acuerdo: Yehimy le avisará vía Nibaldo o Guillermo cuando haya material para revisar.

### 7. Referencias de acceso/licencia de Jonatan (reu 25 jun) — como disclaimer
- Jonatan entregó la URL de soporte de GCO (`soporte.gco.global/...#tab-Visual+Studio+2022`) y el flujo real de acceso: SSO `github.com/enterprises/gco-emu/sso` → si falla, `myprofile.microsoft.com` → Mi acceso → Paquetes de acceso → Solicitar acceso a GitHub Copilot (máx. 40 min tras aprobación).
...
| Plan de facturación real de GCO (legacy vs. tokens) | Nibaldo | Pendiente — la tarjeta 1-S2 ya es plan-agnóstica, no bloquea |
...
- Se separó conceptualmente la **fórmula del prompt** (Rol + Contexto + Tarea + Restricciones + Formato, de `estructura_prompt.md`) de las **5 tarjetas de optimización de tokens** (1-S2 a 5-S2, que pertenecen a Pub 4 ya publicada). Pub 8/9 tratan la fórmula, no las tarjetas de tokens.
...
- La corrección de URLs de acceso/licencia (Jonatan) se deja como **disclaimer no aplicado**, sin borrar ni mover lo existente — se implementa cuando se decida.
...
- **Aprobaciones** (reunión 30 jun): Jonatan no recibe notificaciones en el Teams de Raona → se le avisa vía Nibaldo o Guillermo.
...
- **Producción gráfica en Figma:** 2 tarjetas de Semana 5 — `S5-Card_pub8-formula` y `S5-Card_pub9-contexto-restricciones`.
...
- **Guion del flujo de acceso/licencia:** Yehimy puede ir armándolo desde acá con la doc de Jonatan (insumo Video 1 + Primeros Pasos) — tarea sin bloqueo.
...
- **Aprobaciones:** al tener material para revisión, avisar a Jonatan vía Nibaldo o Guillermo (ya no solo etiquetándolo en Teams de Raona).
...
| `Seguimiento/seguimiento-videos-y-acceso_30jun2026.md` | Tareas de videos + acceso/VPN + **disclaimer de corrección** con las URLs de acceso/licencia de Jonatan |
| `Produccion/semana-5_29jun-03jul/pub-08_mar-30jun_(repub-09_mar-11-ago)/canal-anuncios-tips_pub8.md` | Brief Pub 8 — fórmula del prompt parte 1 (caso SQLCODE COBOL) |
| `Produccion/semana-5_29jun-03jul/pub-09_jue-02jul_(repub-10_jue-13-ago)/canal-anuncios-tips_pub9.md` | Brief Pub 9 — fórmula del prompt parte 2 (Contexto y Restricciones) |
| `Produccion/TABLERO.md` | Estado actualizado al 30 jun — Semana 5 planificada, Pub 8/9 reubicadas |
| `Produccion/semana-4_22-26jun/pub-07_jue-25jun_(repub-08_jue-06-ago)/canal-casos-de-uso_pub7.md` | Pub 7 (caso de uso 2 — bug fixing) · brief + 3 tarjetas listos · pendiente confirmar publicación y lenguaje |
