...
Archivo: `Produccion/semana-5_29jun-03jul/pub-08_mar-30jun/canal-anuncios-tips_pub8.md`
...
- **Pieza 2 — Card del ejemplo** (`08-S5-card-contexto.jpg`): es la card "El mismo encargo, dos resultados" (antes/después). Se reescribió el brief completo con los dos paneles (Prompt incompleto / Prompt con fórmula), bloques de código anotados por pieza y los bloques **Resultado + Eficiencia**.
- **Separación pedida por Yehimy:** se eliminó la sección "Ejemplo real…" que mezclaba diseño y texto. Ahora el antes/después vive en el **brief de la pieza 2** y el **Reply 1** quedó como pie de foto corto que complementa la card sin duplicarla.
...
### 2. Pub 9 — díptico producido y reply único de alto valor
Archivo: `Produccion/semana-5_29jun-03jul/pub-09_jue-02jul/canal-anuncios-tips_pub9.md`
- Se produjo como **díptico de dos cards horizontales** (~1920×1080), no una sola card comparativa:
  - **Card 1 — Contexto** (`9-S5-card-contexto.jpg`): orden de referencias (`#selection` → método/clase → `#file` → `@workspace`) + panel "Menos tokens, mayor enfoque" con terminal mainframe (SQLCODE -911 / SQLSTATE 40001 / DEADLOCK).
  - **Card 2 — Restricciones** (`9-S5-card-restricciones.jpg`): COBOL columnas 7–72, SQL sin LIKE/DELETE/TRUNCATE, .NET C# 12 async/await, cierre "nunca credenciales en el código".
- Brief, tablas y reparto reescritos para reflejar el díptico real.
- **Reply único creado** (a pedido de Yehimy — "que genere mucho valor"): "Escribe las reglas una vez, no en cada prompt" — el salto a `.github/copilot-instructions.md` con un bloque de ejemplo copiable armado con las reglas reales de GCO. Se consolidaron en este reply los dos replies anteriores (instructions.md + ejemplo .NET); el `async/await` quedó dentro del ejemplo.
...
- **Remapeo:** las cards `1-S2` a `5-S2` (optimización de tokens) se corrigieron de "Pub 8 / 9" a **Pub 4** en el tablero (pertenecen a Pub 4, ya publicada; Pub 8/9 tratan la fórmula del prompt).
...
- **Pub 8 lleva dos piezas gráficas** con roles distintos: infografía de la fórmula (post principal) + card del ejemplo antes/después (Reply 1). El ejemplo dejó de ser "solo texto".
- **Pub 9 es un díptico** de dos cards horizontales, no una card comparativa única.
- **Pub 9 lleva un solo reply** (consolidado), enfocado en `copilot-instructions.md` con ejemplo copiable de reglas reales de GCO. El antes/después de .NET se puede recuperar como segundo reply si se pide.
...
- **No se editaron** documentos fechados/snapshot: `Seguimiento/resumen-reunion_30jun2026.md` (aún menciona nombres de card antiguos como "pendiente esta semana" — correcto para su fecha) ni los archivos de `_historico/`.
...
- **Decisión de Yehimy (pendiente):** corregir "Github Copilot" → "GitHub Copilot" en el cuerpo de las cards (regla de marca del proyecto).
- **Opcional:** sumar un segundo reply a Pub 9 con el antes/después específico de .NET (está en el historial).
...
| Archivo | Para qué |
|---------|----------|
| `Produccion/semana-5_29jun-03jul/pub-08_mar-30jun/canal-anuncios-tips_pub8.md` | Pub 8 — briefs de las 2 piezas + posts + reply, sincronizado con las imágenes |
| `Produccion/semana-5_29jun-03jul/pub-09_jue-02jul/canal-anuncios-tips_pub9.md` | Pub 9 — brief del díptico + post + reply único de valor |
