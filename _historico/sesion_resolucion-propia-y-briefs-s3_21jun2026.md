...
- Registrado el cambio en `Produccion/semana-2_08-12jun/pub-04_jue-11jun_(repub-05_mar-28-jul)/ajuste-facturacion-tokens_pub4.md` (documento de cambio: qué se cambió, por qué, y punto por punto qué corregir en el post y en la tarjeta).
- **Solo la tarjeta `1-S2-Infografia_pub4` cambia de forma obligatoria** — se reformuló a lenguaje **plan-agnóstico** ("modelos ligeros / avanzados", "bolsa mensual de uso") que es durable sin importar el plan exacto de GCO. Tarjetas 3, 4 y 5 no cambian; 2-S2 quedó como ajuste de consistencia.
- Consolidado `canal-anuncios-tips_pub4.md` para reflejar lo realmente publicado: post con "ligeros/avanzados", tabla Ligero/Avanzado, hilo reestructurado (Respuesta 1 = tarjeta 2-S2 con texto planificador-ejecutor, sin repetir la 1-S2; texto "cuánto consume" retirado).
...
- Pub 6: pasos del post + Respuesta 1. Pub 7: cuerpo del post.
...
- **Secciones internas "Enfoque y tono del caso de uso"** alineadas: qué debe resaltar cada pieza (post / tríptico / respuestas).
...
- **Pub 6 PUBLICADA** en el canal Casos de Uso Reales: post principal + tríptico (3 tarjetas) + respuesta "Prompts listos para el equipo". Respuesta 2 (análisis de impacto, opcional) pendiente. Info validada contra el brief — todo coincide. El **diseño de las tarjetas lo determina Yehimy**: no se observan estilos, solo se valida información (memoria `feedback_diseno-cards-determinado`).
...
| Plan de facturación real de GCO (¿legacy 0x/1x/3x o nuevo por tokens?) | Nibaldo | Pendiente — la tarjeta 1-S2 ya es plan-agnóstica, así que no bloquea |
...
- De las 5 tarjetas de tokens, **solo la 1-S2 cambia obligatoriamente**. Las demás no se tocan.
- El hilo de Pub 4 **no repite la tarjeta 1-S2**: va en el post principal; Respuesta 1 lleva la tarjeta 2-S2 con el texto del planificador-ejecutor.
...
- **Pub 7** — pendiente de publicar (jueves 25 jun). Brief y tríptico listos; falta solo producir las 3 tarjetas en Figma.
- **Respuesta 2 de Pub 6** (análisis de impacto, opcional) — pendiente de publicar si se quiere reforzar.
- (Pendiente de decisión del equipo) Publicar Respuestas 2/3/4 del hilo de Pub 4 en Teams.
...
| `Produccion/semana-2_08-12jun/pub-04_jue-11jun_(repub-05_mar-28-jul)/ajuste-facturacion-tokens_pub4.md` | Registro del cambio de la tarjeta 1-S2 — qué cambió, por qué, punto por punto |
| `Produccion/semana-2_08-12jun/pub-04_jue-11jun_(repub-05_mar-28-jul)/canal-anuncios-tips_pub4.md` | Contenido final publicado de Pub 4 (post + hilo consolidado) |
| `Produccion/semana-4_22-26jun/pub-06_mar-23jun_(repub-07_mar-04-ago)/canal-casos-de-uso_pub6.md` | Pub 6 — COBOL, entender código heredado · **PUBLICADA** (post + tríptico + respuesta prompts) |
| `Produccion/semana-4_22-26jun/pub-07_jue-25jun_(repub-08_jue-06-ago)/canal-casos-de-uso_pub7.md` | Brief Pub 7 — bug fixing con contexto de negocio |
| `Produccion/TABLERO.md` | Estado actualizado al 21 jun — Pub 6 publicada, Pub 7 lista, trípticos `S3-Card_*`, tarjeta 1-S2 reformulada |
