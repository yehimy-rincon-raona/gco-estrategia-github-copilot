...
> Estado: ✅ **PUBLICADA** — post principal + tríptico (3 tarjetas) + respuesta "Prompts listos para el equipo" (verificado contra la captura de Teams: `Produccion/imagenes_teams/04_casos-de-uso-reales/`) · sigue pendiente confirmar con Nibaldo si se etiqueta como **.NET**
...
**Asunto:** `Caso de uso #2: un bug que no estaba donde parecía`

> Actualizado contra la captura: el título publicado lleva la numeración **"#2"** y no muestra emoji (el borrador decía "🟥 Caso de uso:" sin número).

---

**Caso de uso #2: un bug que no estaba donde parecía**
...
- **Respuestas del hilo:** las tres capas de contexto y el puente con MCP.
...
- Post, flujo de 4 pasos, prompts, tríptico y respuesta. **La publicación es autosuficiente** si se titula como "bug fixing con contexto de negocio" (sin atar a un lenguaje). El post no menciona ".NET" en el cuerpo, así que ya es publicable tal cual.
...
### Respuesta 2 — (opcional) el puente con MCP
...
## Brief — tríptico Pub 7 (`08-S3-Card_pub7-*`) · card-triptico · 3 paneles (carrusel)
