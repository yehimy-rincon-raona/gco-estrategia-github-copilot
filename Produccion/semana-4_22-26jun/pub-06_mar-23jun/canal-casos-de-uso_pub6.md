...
> Estado: ✅ **PUBLICADA** — post principal + tríptico (3 cards) + reply "Prompts listos para el equipo". Reply 2 (análisis de impacto, opcional) pendiente. Info validada contra el brief.
...
**Asunto:** `🟦 Caso de uso: entender un COBOL heredado antes de tocarlo`

---

🟦 **Caso de uso: entender un COBOL heredado antes de tocarlo**
...
- **Reply 1:** la clave del caso (entender antes de tocar) + 2 prompts para copiar.
- **Reply 2 (opcional):** el análisis de impacto del copybook.

**En la pieza gráfica (tríptico · carrusel de 3 paneles):**
- **`S3-Card_pub6-portada`:** la pregunta-gancho del escenario.
- **`S3-Card_pub6-problema`:** el dolor real — tocar un campo y no saber qué rompes.
- **`S3-Card_pub6-solucion`:** el método (explica → planifica → valida) con el **análisis de impacto del copybook** como remate.
...
> Para incluir en el hilo (Reply) o en el post. Son la **técnica** demostrada en sesión, lista para copiar. El developer reemplaza lo que está entre corchetes.
...
- **Replies:** la técnica copiable (prompts) y el dolor real (tocar un campo y no saber qué rompes).
...
- Post completo, flujo de 3 pasos, prompts de ejemplo, tríptico y reply. La publicación es **autosuficiente** — se puede producir y publicar tal cual.
...
> El post principal va con el tríptico `S3-Card_pub6-portada` / `-problema` / `-solucion` (carrusel de 3 paneles: pregunta → problema → solución). Reply 1 aterriza los prompts. Reply 2 (opcional) matiza el análisis de impacto.

---

### Reply 1 — acompaña el tríptico `S3-Card_pub6` (portada · problema · solución)
...
→ Adjuntar: tríptico completo — `S3-Card_pub6-portada`, `S3-Card_pub6-problema`, `S3-Card_pub6-solucion`

---

### Reply 2 — (opcional) profundiza el análisis de impacto
...
|---|-------------------|------|------|
| 1 | `S3-Card_pub6-portada` | card-triptico · panel 1/3 · 1080×1080 px | Portada — pregunta-gancho del escenario |
| 2 | `S3-Card_pub6-problema` | card-triptico · panel 2/3 · 1080×1080 px | Problema — tocar un campo y no saber qué rompes |
| 3 | `S3-Card_pub6-solucion` | card-triptico · panel 3/3 · 1080×1080 px | Solución — método + análisis de impacto del copybook |
...
## Brief — tríptico Pub 6 (`S3-Card_pub6-*`) · card-triptico · 3 paneles (carrusel)
...
### Panel 1 — Portada · `S3-Card_pub6-portada` · `1/3`
...
### Panel 2 — El problema · `S3-Card_pub6-problema` · `2/3`
...
### Panel 3 — La solución · `S3-Card_pub6-solucion` · `3/3`
