...
>
> **Por qué se cambia:** el modelo `0x`/`1x`/`3x` (multiplicadores · "bolsa de peticiones premium") quedó **legacy el 1 jun 2026**. GitHub Copilot pasó a facturación por uso (AI Credits consumidos por tokens). Ver `Conocimiento/facturacion-github-copilot-2026.md`. Es lo que levantó Jonatan en el canal colaborativo.
>
...
> El cambio de facturación afecta **solo a la tarjeta `1-S2` (Optimización de Tokens y Modelos)** y a los textos del post que hablan de coste por modelo. **Las demás tarjetas no se tocan.**

| Tarjeta | Tema | ¿La toca este ajuste? |
|------|------|------------------------|
| `1-S2` · Optimización de Tokens y Modelos | multiplicadores 0x/1x/3x | ✅ **SÍ** — única tarjeta afectada |
| `2-S2` · Diseñe la solución con un modelo Premium | planificador-ejecutor | ✅ Sí — cambiar 2 palabras en la tarjeta: "Premium" → "avanzado", "Estándar" → "ligero" (+ tiene su propio Respuesta 2, ver abajo) |
...
**En el texto del post:** cambian el cuerpo del post, el Respuesta 1 (texto, **sin tarjeta**) y la procedencia. El Respuesta 2 tiene texto nuevo + la tarjeta `2-S2` con 2 palabras cambiadas. Los Respuestas 3, 4 y 5 **no se tocan**. Ver **Orden del hilo** abajo.

---

## ORDEN DEL HILO EN TEAMS

> Aclaración de qué tarjeta lleva cada publicación, para no repetir ni descuadrar. **La tarjeta `1-S2` va en el post principal; cada respuesta lleva una tarjeta distinta.** Refleja lo realmente publicado.

| Publicación | Texto | Tarjeta adjunta | Estado |
|-------------|-------|-----------------|--------|
| **Post principal** | Cuerpo del post (puntos 1–4) | ✅ Tarjeta `1-S2` "Menos/Más Consumo" | ✅ Publicado |
| **Respuesta 1** | "Diseña con el que razona…" (punto 6) | ✅ Tarjeta `2-S2` "Diseñe la solución con un modelo Avanzado" | ✅ Publicado |
| **Respuesta 2** | Texto Skills | Tarjeta `3-S2` "Carga progresiva de Skills" | ✅ Publicado (verificado en captura Teams) |
| **Respuesta 3** | Texto memoria | Tarjeta `4-S2` "Higiene de la memoria" | ✅ Publicado (verificado en captura Teams) |
| **Respuesta 4** | Texto memoria vs instrucciones | Tarjeta `5-S2` "Memoria vs. Instrucciones" | ✅ Publicado (verificado en captura Teams) |

> Nombre de la tarjeta del post principal corregido contra la captura: las columnas de la pieza publicada dicen **"Menos Consumo" / "Más Consumo"** (antes este registro decía "Menos/Mayor Consumo").

> **Nota:** el texto "cuánto consume" (punto 5) **no se publicó como respuesta separado** — el post principal + su tarjeta `1-S2` ya cubren esa idea. Se conserva en el punto 5 como registro, por si en el futuro se quiere usar como comentario aparte.

---

## CAMBIOS EXACTOS A APLICAR en `canal-anuncios-tips_pub4.md`

> Formato: **DÓNDE** · texto actual → texto nuevo. Puntos 1–5 y 7 = facturación (post principal + Respuesta 1 + tarjeta `1-S2`). Punto 6 = Respuesta 2 (texto propio + tarjeta `2-S2` con 2 palabras cambiadas).
...
### 5. Respuesta 1 (acompaña `1-S2-card_pub4`)
...
### 6. Respuesta 2 — post propio + tarjeta `2-S2`

> Este Respuesta va con la tarjeta `2-S2` ("Diseñe la solución…"). Texto definitivo:
...
> **Tarjeta que acompaña este Respuesta:** `2-S2` con 2 palabras cambiadas en el título → "Diseñe la solución con un modelo **avanzado**. Y ejecute el código con uno **ligero**." Los bullets `#file` y `@workspace` no se tocan.

---

### 7. Brief `1-S2-card_pub4` · card-comparativa — contenido

Esta tarjeta es la que más cambia.

**Lo que la tarjeta tiene HOY (transcripción de la pieza producida):**
...
**Quitar de la tarjeta:** los badges `0x` y `1x | 3x`, los precios "(Gratis)" y "1x - 3x", y los nombres de modelo "GPT-4.1 / 5 mini" y "Claude / Gemini" (no listar modelos sin confirmar cuáles tiene GCO).

---

### 7.1 — Correcciones sobre la tarjeta YA PRODUCIDA (imagen del 21 jun)

> Mapeo directo de lo que se ve en la tarjeta actual → cómo debe quedar. Pensado para corregir en Figma sin rediseñar.

**🔴 Obligatorias (lo que está factualmente mal):**


| En la tarjeta ves... | Corrige a... |
|-------------------|--------------|
...
> **Nota de diseño:** los números gigantes `0x` y `1x | 3x` eran el foco visual de la tarjeta. Al reemplazarlos por etiquetas de texto ("Menor/Mayor consumo"), el peso visual baja — el diseñador puede agrandar esas etiquetas o usar un indicador gráfico (ej. una barra de "consumo") para mantener el impacto.

> **Tarjeta `2-S2-card_pub4` ("Diseñe la solución con un modelo Premium") — cambio confirmado:** en el título cambiar "Premium" → "avanzado" y "Estándar" → "ligero" → queda "Diseñe la solución con un modelo **avanzado**. Y ejecute el código con uno **ligero**." Los bullets `#file` y `@workspace` no se tocan. Esta tarjeta acompaña al Respuesta 2 (ver punto 6).
...
1. **Plan de las licencias de GCO** (Business / Enterprise) y si ya están en usage-based (AI Credits) o en plan anual legacy. Define si la tarjeta puede incluir cifras concretas o se queda conceptual (recomendado: conceptual).
